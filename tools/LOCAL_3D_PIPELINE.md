...
### 商業レベルに上げる作り込み技法（bl_structures.py で実証）
プリミティブ積みは安っぽい。以下で「作り込み」感を出す:
- **revolve(回転体/Screwモディファイア)**: 断面プロファイル(x,z)をZ軸回転。トーラス/スコチア/オジー曲線などの
  繰形(モールディング)が一発で出る。柱・バラスター・聖火台・ドーム・フィニアル・丘・島すべてこれが主役。
  プロファイルは始端/終端をx=0(軸上)にして上下を閉じる。`use_merge_vertices=True`必須。
- **フルート溝**: 細い円柱を周囲にN本並べて結合→1回のブーリアン差分で彫る。
- **装飾リング**: エッグ&ダート(球と尖りの交互)、ヴーソワール(楔石を半円配置)、ストリングコース(金トーラス)。
- **サブサーフ**で滑らかに。仕上げに弱いベベル。
- **窓/ドアは穴を開けない**: ブーリアンで本体を抜くと「no primitives」で本体が消える事故が起きやすい。
  発光ガラス+凹み暗パネル+金アーチ枠を**表面に貼る**ユニット方式が堅実(外から見れば窓に見える)。
- **つる草/花**: 螺旋状に小さなicosphereを多数配置。遠目に有機的なディテールになる。

### Tripo(課金 text-to-3D)の位置づけ
有機的なヒーロー構造物は一発で高密度。ただしクレジット有限なので**温存**。自作で届く範囲はローカルで作る。
1点生成済みの妖精城塔は struct_tower(遠景)に活用済み。自作の struct_fairytower(192KB)はTripo塔(2.3MB)と
同等の見栄えを軽量に実現できており、基本は自作で足りる。
