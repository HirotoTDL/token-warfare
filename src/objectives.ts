...
// この秒数内に撃たれていれば「占領中」=固定レート加算(武器連射差を吸収)。
// 【2026-06-16 チャージャー追加に伴い 1.3→1.5】最低連射のジン(1.22s間隔)に加え、garoのチャージャー(フル溜め≈1.37s周期)でも
// スフィアへ撃ち続ければ占領が途切れないようウィンドウを僅かに広げた(発射間隔の長い武器でも占領を維持できる)。
export const CAPTURE_HIT_WINDOW = 1.5
export const CAPTURE_TO_WIN = 30 // 勝利カウント
export const SPHERE_RADIUS = 1.6 // 当たり判定/見た目半径
// 基地スフィアをスポーン点から中央寄りへ前進させる量(m)。リスキル一強の根本対策=勝利目標とスポーン点を非同一座標化。
export const BASE_SPHERE_FORWARD = 9
...
export class Objectives {
  ...
  constructor(scene: THREE.Scene, centerPos: THREE.Vector3, basePos: Record<Team, THREE.Vector3>) {
    ...
    // 基地スフィアはスポーン点(basePos)そのものでなく中央寄りへ前進させる=勝利目標とスポーン点を分離し、
    // 「敵を倒す→敵スポーン(=敵基地スフィア)に居座る」リスキル一強を構造的に防ぐ(実在FPSの大原則: Halo/CoD)。
    // スポーン点(basePos)自体は不変。基地スフィアだけ約9m中央寄りに置き、復帰した被害者がスフィア手前の遮蔽帯で再合流できる。
    const forward = (sp: THREE.Vector3): THREE.Vector3 => {
      const p = sp.clone().setY(centerPos.y)
      const toCenter = new THREE.Vector3(centerPos.x - p.x, 0, centerPos.z - p.z)
      const d = toCenter.length()
      if (d > BASE_SPHERE_FORWARD) p.add(toCenter.multiplyScalar(BASE_SPHERE_FORWARD / d))
      return p
    }
    const blueBase = new Sphere('blueBase', forward(basePos.blue), 1, 'blue')
    const redBase = new Sphere('redBase', forward(basePos.red), -1, 'red')
