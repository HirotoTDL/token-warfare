...
export function buildArena(world: World, mapKey = 'skyhaven', lite = false) {
  ...
  if (mapKey === 'neondocks') {
    ...
    // --- 3レーン構造(クリーンな街路設計) ---
    // 散らかった小物(クレート/バレル)の乱立で通路を作るのを廃し、『隙間のない連続壁＋意図的な開口(doorway)』で
    // 中央レーン(|x|<10=正面突破)と左右フランクレーン(|x|>10=裏どり)を明確に区切る。横移動は出入口だけ(=計画的な抜け)。点対称。
    for (const sx of [1, -1]) {
      // 縦の仕切り壁(x=±10): 連続した実体壁。z=±15付近に出入口を2つだけ開け、フランク⇄中央の横移動(迎撃)を許す。
      wall(sx * 10, 24, 0.9, 12, 2.6)  // 青基地側セグメント z∈[18,30]
      wall(sx * 10, 0, 0.9, 24, 2.6)   // 中央セグメント z∈[-12,12](デッキ前を縁取る)。← doorway: z∈[12,18]とz∈[-18,-12]
      wall(sx * 10, -24, 0.9, 12, 2.6) // 赤基地側セグメント z∈[-30,-18]
    }
    // 基地スフィア前の遮蔽(中央高所デッキからの撃ち下ろしを1本切る。占領側が身を隠せる。点対称)
    wall(0, 26, 9, 0.9, 1.8); wall(0, -26, 9, 0.9, 1.8)
    // 各フランクレーンの実体ピークカバー(乱立させず1レーン1〜2個だけ・点対称)
    wall(18, 6, 3.4, 0.9, 1.6); wall(-18, -6, 3.4, 0.9, 1.6)
    wall(20, -16, 0.9, 4, 1.6); wall(-20, 16, 0.9, 4, 1.6)
  ...
  // 28MB→最適化済GLB。配置時に未ロードなら、ロード完了まで毎フレーム再試行する。
  {
    ...
    // 中央デッキを囲むかがり火(発光のランドマーク)。十字(レーン中央線上)だと通路を塞ぐので、
    // デッキ対角コーナー(±8,±8)に置いて中央/フランクレーンを通す。
    for (const [bx, bz] of [[8, 8], [-8, -8], [8, -8], [-8, 8]] as const) {
