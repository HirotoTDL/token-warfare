...
export function buildArena(world: World, mapKey = 'skyhaven', lite = false) {
  ...
  if (mapKey === 'neondocks') {
    ...
    // --- 街区の3レーン化(導線設計): 中央レーン(|x|<8=正面突破)と左右フランクレーン(|x|>11=裏どり)を
    // 縦の仕切り壁で分け、z方向に隙間を空けて横移動(迎撃=lane swap)の抜け道を作る。点対称配置。 ---
    for (const sx of [1, -1]) {
      // 縦の仕切り壁(x≈±9.5)。z=±22 と z=±11 にセグメント、間(z≈±16)と中央(z=0近傍)はギャップ=横の抜け
      wall(sx * 9.5, 22, 0.9, 7, 2.4); wall(sx * 9.5, -22, 0.9, 7, 2.4)
      wall(sx * 9.5, 11, 0.9, 4.5, 2.0); wall(sx * 9.5, -11, 0.9, 4.5, 2.0)
      // フランクレーンの外縁ガイド(x≈±27)。基地〜フランクの通りを縁取り、長射線を切る
      wall(sx * 27, 0, 0.9, 7, 2.2)
    }
